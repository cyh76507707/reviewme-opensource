...
contract ReviewMe_v0_01 is ReentrancyGuard {
    using SafeERC20 for IERC20;

    address public constant REVIEWME_TOKEN = 0xdF9CC5574a70435bF34D78E8e1698cc52f436bB7;
    ...
    function submitReview(
        ...
        // Use tx.origin to get the actual transaction initiator (the real reviewer),
        // even when called through Decent.xyz or other intermediate contracts.
        // This is safe for user tracking (not authorization), similar to Signet's approach.
        address actualReviewer = tx.origin;
        require(reviewee != actualReviewer, "Cannot review yourself");

        // Receive HUNT tokens from msg.sender (the caller)
        // When called directly: msg.sender = user wallet
        // When called through Decent: msg.sender = UTBExecutor (which holds the swapped HUNT)
        IERC20(HUNT_TOKEN).safeTransferFrom(msg.sender, address(this), maxHuntAmount);
        IERC20(HUNT_TOKEN).approve(MINTCLUB_BOND, maxHuntAmount);
        ...
        // Record the review with the actual reviewer (tx.origin)
    ...
    function getReviewsForWallet(address wallet) external view returns (Review[] memory, uint256[] memory) {
        uint256[] memory reviewIds = reviewsByReviewee[wallet];
        Review[] memory walletReviews = new Review[](reviewIds.length);
        
        for (uint256 i = 0; i < reviewIds.length; i++) {
            walletReviews[i] = reviews[reviewIds[i]];
        }
